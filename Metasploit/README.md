...
Then we can select the `TRUN` module with the [`use`](https://www.offsec.com/metasploit-unleashed/msfconsole-commands/#use) command followed by the index of the module we want to use. In my case this was `9`.
...
1. Show options
   ...
   * The *Module Options* are those specific to our module. The `LOWERBOUND` and the `UPPERBOUND` configurations sets the lower and upper range of our search space respectively. The `RETOFFSET` option is a custom option for telling the exploit the offset to the *return* value on the stack. The other options, `RHOSTS` and `RPORT` are use to control the remote connections our exploit will make and are from the ` Msf::Exploit::Remote` class we inherit.
     * We also have the `USERNAME` configuration used with the telnet mixin, if present this can be left blank.
...
> [!IMPORTANT] 
> You will need to replace the ROP chain function in the Metasploit code. The rop_chains.txt generated by mona.py will contain the ruby function you can modify for the chain we are guessing the base address of.
